...
Attention: Any party changing the contents of the LIF which may affect or be expected to affect mobile robot behavior (e.g., change of layout) without also supplying the same LIF to other parties and confirming their incorporation of it can lead to potentially harmful or destructive outcomes. Any party which changes a facility's LIF definition and updates only a single component accepts all liability for negative outcomes.
